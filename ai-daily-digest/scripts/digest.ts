...
// ============================================================================
// Report Generation + JSON output (for web/API)
// ============================================================================

/** Serializable digest payload (pubDate as ISO string) */
...
function buildDigestJson(articles: ScoredArticle[], highlights: string, stats: {
  totalFeeds: number;
  successFeeds: number;
  totalArticles: number;
  filteredArticles: number;
...
async function main(): Promise<void> {
  ...
  const report = generateDigestReport(finalArticles, highlights, stats);
  const digestJson = buildDigestJson(finalArticles, highlights, stats);

  await mkdir(dirname(outputPath), { recursive: true });
  await writeFile(outputPath, report);

  const jsonPath = outputPath.replace(/\.md$/i, '.json') || `${outputPath}.json`;
  await writeFile(jsonPath, JSON.stringify(digestJson, null, 2));
